...
class MicaSenseAnalyzer:
    ...
    n_clusters           : maximum K-means clusters
    ...
    def find_stress_clusters(
        ...
        for k in range(n_k):
            ...
            # ── radius-based health gate ──────────────────────────────
            is_large = radius_m > self.min_cluster_radius_m
            if is_large:
                health = "UNHEALTHY"
    ...
    def run(
        ...
        # 13. Transmit each cluster-centre GPS to the ground computer over serial.
        print(f"\n{sep}\nStep 10 — Transmit cluster GPS over serial "
              f"({self.serial_port} @ {self.serial_baud} baud)")

        # Collect every cluster centre across all indices.
        gps_targets = [
            (row["lat"], row["lon"])
        ...
        if not gps_targets:
            print("  No clusters to transmit.")
        else:
            try:
                with serial.Serial(self.serial_port, self.serial_baud,
                                   timeout=1) as ser:
                    if self.serial_settle_s:
                        time.sleep(self.serial_settle_s)  # let the link/receiver settle
                    for lat, lon in gps_targets:
                        payload = f"{lat:.6f},{lon:.6f}"
                        msg     = f"{payload}*{self._nmea_checksum(payload)}\n"
                        ser.write(msg.encode("utf-8"))
                        print(f"  Sent: {msg.strip()}")
                print(f"  Transmitted {len(gps_targets)} cluster centre(s).")
            except serial.SerialException as e:
                print(f"  Error sending GPS coordinates: {e}")


        # 13. Send GPS coordinates to ground computer through serial port
        for cluster_info in clusters.values():
            df_cluster, health = cluster_info
            if not df_cluster.empty:
                for _, row in df_cluster.iterrows():
                    lat = row['lat']
                    lon = row['lon']
                    gps_message = f"{lat:.6f},{lon:.6f}\n"
                    try:
                        with serial.Serial('/dev/ttyUSB0', 57600, timeout=1) as ser:
                            ser.write(gps_message.encode('utf-8'))
                            print(f"Sent GPS coordinates to ground computer: {gps_message.strip()}")
                    except serial.SerialException as e:
                        print(f"Error sending GPS coordinates: {e}")
...
if __name__ == "__main__":
    ...
    analyzer = MicaSenseAnalyzer(
        camera_ip             = "http://192.168.10.254",
        cache_format          = "jpeg",      # "jpeg" (fast 8-bit) or "tif" (16-bit)
